<template>
  <div>
    ...
    <Tab ref="tab" title="特性调查数据" :tableData="tableData" :headerList="headerList" :request="request"
      ...
      <template slot="select">
        <el-form :inline="true">
          ...
          <el-form-item>
            <el-input placeholder="请输入特性调查名称" v-model="request.data.wasteName" size="mini" clearable
              @input="e => e ? '' : requestFn('reset')"></el-input>
          </el-form-item>
          ...
          <el-form-item>
            <el-button v-has="{ action: '/tech/character/add', effect: 'disabled' }" type="primary" size="mini"
      ...
      <template slot="column">
        <el-table-column label="操作" width="250px" align="left">
          <template slot-scope="scope">
            ...
            <el-button v-has="{ action: '/tech/character/update', effect: 'disabled' }" type="primary" size="mini"
              @click="addOrUpdateOrDelete('编辑', scope.row)">编辑</el-button>
            <el-button v-has="{ action: '/tech/character/delete', effect: 'disabled' }" type="danger" size="mini"
    ...
    <Dialog :visible="visible" :title="title" @submit="wasteSubmit()" @show="show()">
      <template>
              ...
              <el-col :span="12">
                <el-form-item label="设施名称:" prop="buildVo.buildId">
                  <el-select v-model="form.buildVo.buildId" placeholder="请选择设施" default-first-option>
                    <div class="option">
                      <el-option v-for="       item        in        parentArr       " :key="item.buildId"
                        :label="item.buildName" :value="item.buildId"></el-option>
                    </div>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="特性调查名称:" prop="wasteName">
...
import {
  // 所有项目
  prjBuildGetList,
  techCharacterAdd as add,
  techCharacterUpdate as update,
  techCharacterPage as requestFn,
  techCharacterDelete as deleteFn,
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        wasteName: [
          { required: true, message: "特性调查名称不能为空", trigger: "blur" },
        ],
        nums: [
          {
            // required: true,
  ...
  methods: {
    ...
    async prjBuildGetList() {
      const res = await prjBuildGetList({
        data: {},
        page: 1,
        limit: 1000,
      });
