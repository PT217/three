...
class FlyControls extends EventDispatcher {
	...
	constructor(object, domElement) {
		...
		this.keydown = function (event) {
			...
			if (event.altKey || document.activeElement.tagName.toLowerCase() === 'input') return
			...
			switch (event.code) {
				...
				case 'KeyC': this.moveState.rollRight = 1; break;
		...
		this.keyup = function (event) {
			...
			switch (event.code) {
				...
				case 'KeyC': this.moveState.rollRight = 0; break;
		...
		// 在构造函数中添加窗口失去焦点时的事件监听
		this.blur = function () {
			for (const key in this.moveState) {
		...
		this.domElement.addEventListener('mouseenter', () => this.dragToLook = true);
